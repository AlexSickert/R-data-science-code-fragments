...
# parallelize and use ngram properly
...
MAIN_LOOP <- function(){
  ...
  for(loopCount in 1:numberOfLoops){
    ...
    print("making ngrams... ")
    parallelizeTask(makeNgram, currentToken)
    ...
    parallelizeTask(fillPredictionTables)
...
makeNgram <- function(t){
  for(i in 1:ngramDepth){  
    ...
    # ngramList[[i]]  <<- parallelizeTask(makeOneNgram, t, i)
...
makeOneNgram <- function(t, i){
  ...
  # ngram <- quanteda::ngrams(t, n = 6)
  # print("ngram ... ")
  # print(ngram)
  
  # ngram <- quanteda::ngrams(t, what = "word", removeNumbers = TRUE,
           # removePunct = TRUE, removeSeparators = TRUE,
           # removeTwitter = FALSE, removeHyphens = TRUE,
           # ngrams = i, simplify = TRUE)
...
makeToken <- function(currentcorpus){
  ...
  tokens <- quanteda::tokenize(toLower(unlist(currentcorpus)), what = "sentence", removeNumbers = TRUE,
           removePunct = TRUE, removeSeparators = TRUE,
  ...
  tokens <- quanteda::tokenize(toLower(unlist(tokens)), what = "word", removeNumbers = TRUE,
                               removePunct = TRUE, removeSeparators = TRUE,
                               removeTwitter = TRUE, removeHyphens = TRUE)
  # print("tokens ... 2")
  # print(tokens)
...
saveFiles <- function(){
  ...
  # }
    ...
    filenameTST <- "export.txt"
      
    if(file.exists(filenameTST)){
      
      for(i in 1:ngramDepth){
        print(Sys.time())
        print("saving RDS file... ")
        filename <- paste("hash-", i, ".RDA",  sep="", collapse="" ) 
        saveRDS(predictionTables[[i]] , file=filename)
        h <- predictionTables[[i]]
        print(Sys.time())
      }
        
      file.remove(filenameTST)
    }else{
      
      print("no file, no file saving...")
    }
      
   
    
    # print("saving txt... ")
    # filename <- paste("hash-", i, ".txt",  sep="", collapse="")
...
prepareTables <- function(){
  ...
  for(i in 1:ngramDepth){
    ...
    if(!file.exists(filename)){
      ...
      predictionTables[[i]] <<- new.env(hash=TRUE)
    }else{
