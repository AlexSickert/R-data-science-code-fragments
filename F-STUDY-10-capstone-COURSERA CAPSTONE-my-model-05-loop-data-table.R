...
prepareTables <- function(){
  predictionTables <<- list() 
  for(i in 1:ngramDepth){
    predictionTables[[i]]  <<- data.table(data.frame( "nGramString" = character(), "searchString" = character(), "prediction" = character(), "frequency" = integer(), stringsAsFactors=FALSE))
    setkey(predictionTables[[i]],nGramString)
...
addRowToTable <- function(depth, row){
  ...
  #searchWord <- row[1, 2]
  searchWord = paste("", row[1, 2], sep="")

  setkey(predictionTables[[depth]],nGramString)
  c <- predictionTables[[depth]][searchWord]   
  tst <- is.na(c[1,searchString]) 
  
  end.time <- Sys.time()
  time.taken <- end.time - start.time
  print(paste("addRowToTable() part 1 took timet:", time.taken, sep=" "))
  ...
  if(tst == FALSE){
    ...
    f <- c[1,frequency]
    f
    predictionTables[[depth]][searchWord,4] <<- f + row[1, 1]     
    #predictionTables[[2]]
    ...
    time.taken <- end.time - start.time
    print(paste("addRowToTable() part 2 took timet:", time.taken, sep=" "))
  }else{
    ...
    printLog(paste("adding:", a, sep=" "))
    printLog(paste("adding:", f[1], sep=" "))
    printLog(paste("adding:", f[2], sep=" "))
    printLog(paste("adding:", row[1, 1], sep=" "))
    
    predictionTables[[depth]] <<- rbind(predictionTables[[depth]],list(a,f[1],f[2],row[1, 1]))
    
    end.time <- Sys.time()
    time.taken <- end.time - start.time
    print(paste("addRowToTable() part 3 took timet:", time.taken, sep=" "))
